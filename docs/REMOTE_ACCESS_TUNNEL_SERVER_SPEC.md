# 远程访问中心服务器（tunnel-server）实现说明

本文档描述**中心服务器（tunnel-server）**需要实现的功能与协议，用于与设备端（隧道客户端）对接。服务端为独立项目，按本文档实现即可与兼容协议的设备端对接。
...
- **设备端（已有）**：在 4G/NAT 后，主动用 WebSocket 连接中心服务器，将「用户通过服务器发来的 HTTP 请求」在设备本地转发到 `http://127.0.0.1:8080`，并把响应通过 WebSocket 发回服务器。
- **中心服务器（待实现）**：
